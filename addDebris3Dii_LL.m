...
% Z1 debris (surface) ------------------------------------------------------------
% z1 face next: you will have (s-1) + 1 (left side) + 1 (right side) quadrilateral faces
z1debris_header = NaN*ones(1 + n_debris-1  ,3); % 3 numbers make up a header
z1debris_facets = NaN*ones(1 + n_debris-1  ,4); % 1 number saying the number of nodes we'll have
                            % and 4 nodes comprising the actual facet
flag = 32;                  % Z1 flag;
...
yTRIface_debris(1,:) = [3  ds1-1 ds1 d1]; %not sure what the first numbers are here
% right:
yTRIhead_debris(2,:) = [1 0 flag];
yTRIface_debris(2,:) = [3 b2 d2 d2-1]; %what does the 3 mean
...
yTRIface_debris(3,:) = [3 ds2-1 d3 ds2];    
% right:
yTRIhead_debris(4,:) = [1 0 flag];
yTRIface_debris(4,:) = [3  d4-1 d4 b4];
